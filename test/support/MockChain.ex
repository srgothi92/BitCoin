defmodule MockChain do
  ...
  def mockLedger do
    ...
    :ok = GenServer.call(:TransactionQueue, {:addToQueue, [suTx]})
    muTx = Transaction.initialDummyTransaction([TxOutput.createTxOutput(mu.address, 20)])
    :ok = GenServer.call(:TransactionQueue, {:addToQueue, [muTx]})
    # first transaction
    ...
    :ok = GenServer.call(:TransactionQueue, {:addToQueue, [tx2]})
     # third transaction
     ...
     :ok = GenServer.call(:TransactionQueue, {:addToQueue, [tx3]})
     ...
     :ok = GenServer.call(:TransactionQueue, {:addToQueue, [tx4]})
